...
Tout d'abord - vos applications Cordova devraient adopter la conception SPA (Single Page Application). Vaguement défini, une SPA est une application côté client qui s'exécute à partir d'une seule demande d'une page web. L'utilisateur charge un ensemble initial de ressources (HTML, CSS et JavaScript), et les différents appels (montrant une nouvelle vue, chargement de données) se font via AJAX. Les SPA sont couramment utilisés pour des applications clientes complexes. GMail est un bon exemple de cela. Après avoir chargé GMail, les vues de courrier, l'édition et organisation sont tout fait en mettant à jour le DOM au lieu de quitter la page en cours pour en charger une nouvelle.

Utiliser une application SPA peut vous aider à organiser votre application de manière plus efficace, mais il a aussi des avantages spécifiques pour des applications de Cordova. Une application de Cordova doit attendre l'événement [deviceready](../../cordova/events/events.deviceready.html) se declencher avant que les plugins puissent être utilisés. Si vous n'utilisez pas le concept SPA, et que votre utilisateur clique pour passer d'une page à l'autre, vous devrez attendre que l'evenement [deviceready](../../cordova/events/events.deviceready.html) se declenche une nouvelle fois avant d'utiliser un plugin. C'est facile d'oublier que votre application s'agrandit.

Même si vous choisissez de ne pas utiliser de Cordova, la création d'une application mobile sans utiliser une architecture de page unique (SPA) aura de graves implications sur les performances. C'est parce que la navigation entre les pages exigera des scripts, actifs, etc., pour être rechargé. Même si ces biens sont mis en cache, il y aura toujours des problèmes de performances.
...
**Clique versus Touch** - l'erreur la plus fréquente que vous puissiez faire est d'utiliser les événements click. Alors que ceux-ci «fonctionnent» très bien sur mobile, la plupart des périphériques imposent un délai de 300 ms sur eux afin de distinguer entre un contact et un événement touch de type "hold" (tenir, appuyez longtemps). L'utilisation de `touchstart` , ou `touchend` , se traduira par une dégradation spectaculaire - 300ms ne ressemble beaucoup, mais elle peut aboutir à des mises à jour de l'interface utilisateur et de comportement saccadé. Vous devriez également considérer le fait que les événements de type « touch » ne sont pas supportés dans les navigateurs non-webkit, voir [CanIUse][10]. Pour remédier à ces limitations, vous pouvez commander différentes bibliothèques comme HandJS ou Fastouch.
...
**Encombrement du réseau** - d'accord, les réseaux n'ont pas tous des performances pitoyables, mais la latence des réseaux mobiles, même de bons réseaux de téléphonie mobile, sont bien pire que ce vous pensez probablement. Une application de bureau qui avale 500 lignes de données JSON, toutes les 30 secondes, seront plus lent sur un appareil mobile et un gros mangeur de batterie. Gardez à l'esprit que les applications de Cordova ont plusieurs façons pour rendre persistantes les données dans l'application (LocalStorage et le système de fichiers, par exemple). Mettez en cache les données localement et soyez conscient de la quantité de données que vous envoyez en arrière plan. Il s'agit d'une considération particulièrement importante lorsque votre application est connectée via un réseau cellulaire.

**Ressources et Articles de performances supplémentaires**

*   [« You half assed it »][11]
*   [« Top Ten Performance Tips for PhoneGap and Hybrid Apps »][12]
*   [« Fast Apps and Sites with JavaScript »][13]
...
Voir le Conseil précédent sur les réseaux. Non seulement vous pouvez être sur un réseau lent, il est tout à fait possible pour votre application d'être complètement déconnecté. Votre application doit gérer cela de manière intelligente. Si votre application ne fonctionne pas, les gens penseront que votre application est inutile. Vu comment il est facile à manipuler l'etat hors connexion (le support de Cordova usr l'écoute pour un événement se fait à la fois en ligne et hors ligne), il n'y a absolument aucune raison pour que votre application ne répond ne pas bien quand elle est exécuté en mode hors connexion. Veillez à tester (voir la section tests ci-dessous) votre demande et n'oubliez pas de tester comment votre application fonctionne quand vous commencez dans un État et ensuite passez à l'autre.

Notez que les événements en ligne et hors ligne, ainsi que l'API de connexion de réseau n'est pas parfaite. Vous devrez compter sur une requête XHR afin de déterminer si l'appareil est vraiment hors ligne ou en ligne. À la fin de la journée, être bien sûr d'ajouter une forme d'assistance pour les problèmes de réseau - en fait, l'Apple store (et probablement d'autres magasins) rejettera les applications qui ne gèrent correctement les États hors ligne/en ligne. Pour plus d'informations sur ce sujet, voir ["Is This Thing sur?"][14]
